...
Do not use file globbing, text grep, or shell search commands like `grep`, `rg`, `find`, or `git grep` for code exploration when the Cog index is available. Those are fallback tools only for missing index coverage or non-symbol text.
